...
def build_lag(raw):
    ...
    # nam = 'mbd_rollmea'
    # raw[nam] = raw.groupby('cfips')['mbd_lag1'].transform(lambda s: s.rolling(ema, min_periods=1).sum())
    # train_col += [nam]
...
def posle_sglashivfnia(raw, rezult, lastactive = 40, max_cfips=100000):
    ...
    kbl = 100 # количество cfips в блек листе blac_cfips
    ...
    # ошибка модели 1.791025, dif_err -0.401444
